...
define('WAW_MEMORY_TO_ALLOCATE',	'100M');
define('WAW_DEFAULT_QUALITY',		90);
define('WAW_CURRENT_DIR',			dirname(__FILE__));
define('WAW_CACHE_DIR_NAME',		'/imagecache/');
$iupload = wp_upload_dir();
define('WAW_CACHE_DIR',$iupload['basedir'] . '/webagency/imagecache/');
define('WAW_DOCUMENT_ROOT',	$_SERVER['DOCUMENT_ROOT']);
...
$docRoot	= preg_replace('/\/$/', '', WAW_DOCUMENT_ROOT);
...
$quality	= (isset($_GET['quality'])) ? (int) $_GET['quality'] : WAW_DEFAULT_QUALITY;
...
$resized		= WAW_CACHE_DIR . $resizedImage;
...
ini_set('memory_limit', WAW_MEMORY_TO_ALLOCATE);
...
if (!file_exists(WAW_CACHE_DIR))
mkdir(WAW_CACHE_DIR, 0755);
// Make sure we can read and write the cache directory
if (!is_readable(WAW_CACHE_DIR))
{
	header('HTTP/1.1 500 Internal Server Error');
	echo 'Error: the cache directory is not readable';
	exit();
}
else if (!is_writable(WAW_CACHE_DIR))
{
	header('HTTP/1.1 500 Internal Server Error');
