...
class Engine_db
{
    ...
    public static function get_instance($params = false)
    {
        ...
        if (!isset(self::$_instance[$instance_name]) or null === self::$_instance[$instance_name])
        {
            if (!is_array($params))
            {
                $dbuser = Engine_config::$dbuser;
                $dbpass = Engine_config::$dbpass;
                $dbname = Engine_config::$dbname;
                $dbhost = Engine_config::$dbhost;
                $dbencoding = Engine_config::$dbencoding;
            }
    ...
    private function __construct($dbuser, $dbpass, $dbname, $dbhost, $dbencoding)
    {
        ...
        if (Engine_config::$db_time_zone)
            $this->connect->query('SET time_zone = \'' . Engine_config::$db_time_zone . '\'');
    }
