...
// default connection
   $waw_dbname = DB_NAME; // Your database name
   $waw_dbuser = DB_USER; // Your database username
   $waw_dbpass = DB_PASSWORD; // // Your database password
   $waw_dbhost = DB_HOST; // Your database host, 'localhost' is default.

require (XCRUD_PATH . '/engine_config.php'); // configuration
require (XCRUD_PATH . '/engine_db.php'); // database class
mb_internal_encoding(Engine_config::$mbencoding); // sets multibyte encoding globaly
date_default_timezone_set(@date_default_timezone_get()); // engine code not depends on timezone, but this can fix some warnings
...
class Engine
{
    ...
    protected $load_view = array(
        'list' => 'engine_list_view.php',
        'create' => 'engine_detail_view.php',
        'edit' => 'engine_detail_view.php',
        'view' => 'engine_detail_view.php');
    ...
    /** constructor, sets basic engine vars (they can be changed by public pethods) */
    protected function __construct()
    {
        Engine_config::$scripts_url = self::check_url(Engine_config::$scripts_url, true);
        Engine_config::$editor_url = self::check_url(Engine_config::$editor_url);
        Engine_config::$editor_init_url = self::check_url(Engine_config::$editor_init_url);

        $this->limit = Engine_config::$limit;
        $this->limit_list = Engine_config::$limit_list;
        $this->column_cut = Engine_config::$column_cut;
        $this->show_primary_ai_field = Engine_config::$show_primary_ai_field;
        $this->show_primary_ai_column = Engine_config::$show_primary_ai_column;

        $this->benchmark = Engine_config::$benchmark;
        $this->start_minimized = Engine_config::$start_minimized;
        $this->remove_confirm = Engine_config::$remove_confirm;
        $this->upload_folder_def = Engine_config::$upload_folder_def;

        $this->theme = Engine_config::$theme;
        $this->is_print = Engine_config::$enable_printout;
        $this->is_title = Engine_config::$enable_table_title;
        $this->is_csv = Engine_config::$enable_csv_export;
        $this->is_numbers = Engine_config::$enable_numbers;
        $this->is_pagination = Engine_config::$enable_pagination;
        $this->is_search = Engine_config::$enable_search;
        $this->is_limitlist = Engine_config::$enable_limitlist;
        $this->is_sortable = Engine_config::$enable_sorting;

        $this->language = Engine_config::$language;

        $this->search_pattern = Engine_config::$search_pattern;

        $this->demo_mode = Engine_config::$demo_mode;

        $this->default_tab = Engine_config::$default_tab;
        $this->prefix = Engine_config::$dbprefix;
        $this->is_rtl = Engine_config::$is_rtl;

        $this->strip_tags = Engine_config::$strip_tags;
        $this->safe_output = Engine_config::$safe_output;

        $this->lists_null_opt = Engine_config::$lists_null_opt;

        $this->date_format = array('php_d' => Engine_config::$php_date_format, 'php_t' => Engine_config::$php_time_format);
    }
    ...
    {
        if (isset($_POST['engine']['instance']) && isset($_POST['engine']['key']) && isset($_POST['engine']['task']))
        {
            self::init_prepare('post');
            $key = $_POST['engine']['key'] ? $_POST['engine']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_POST['engine']['instance'] ? $_POST['engine']['instance'] : self::error('Instance name cannot be empty');
            $is_get = false;
        }
        elseif (isset($_GET['engine']['instance']) && isset($_GET['engine']['key']) && isset($_GET['engine']['task']) && $_GET['engine']['task'] ==
            'file')
        {
            self::init_prepare('get');
            $key = $_GET['engine']['key'] ? $_GET['engine']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_GET['engine']['instance'] ? $_GET['engine']['instance'] : self::error('Instance name cannot be empty');
            $is_get = true;
        }
        else
        {
        ...
        if (isset($_SESSION['lists']['engine_session'][$inst_name]['key']) && $_SESSION['lists']['engine_session'][$inst_name]['key'] ==
            $key)
        {
    ...
    protected static function init_prepare($method = false)
    {
        ...
        {
            case 'post':
                $sess_name = (Engine_config::$dynamic_session && isset($_POST['engine']['sess_name']) && $_POST['engine']['sess_name']) ? $_POST['engine']['sess_name'] :
                    Engine_config::$sess_name;
                break;
            case 'get':
                $sess_name = (Engine_config::$dynamic_session && isset($_GET['engine']['sess_name']) && $_GET['engine']['sess_name']) ? $_GET['engine']['sess_name'] :
                    Engine_config::$sess_name;
                break;
            default:
                $sess_name = Engine_config::$sess_name;
                break;
        }
        self::session_start($sess_name);
        if (is_callable(Engine_config::$before_construct))
        {
            call_user_func(Engine_config::$before_construct);
        }
    }
    ...
    public static function session_start($sess_name = false)
    {
        ...
        if (!session_id() && !Engine_config::$external_session && !Engine_config::$alt_session)
        {
            if (!headers_sent())
            {
        ...
        if (Engine_config::$alt_session)
        {
            if (!headers_sent())
            {
                ...
                setcookie($sess_name, self::$sess_id, time() + Engine_config::$alt_lifetime * 60, '/');
            }
            else
                self::error('xCRUD can not start session, because the output is already sent into browser. 
                Try to define xCRUD instance before the output start or use <strong>Engine::session_start();</strong> at the beginning of your script');
        }
    }
    ...
    public function nested_table($instance_name = '', $field = '', $inner_tbl = '', $tbl_field = '')
    {
        if ($instance_name && $field && $inner_tbl && $tbl_field)
        {
            ...
            foreach ($fdata as $fitem)
            {
                ...
                $instance = Engine::get_instance($instance_name); // just another engine object
                ...
                switch ($type)
                {
                    ...
                    case 'point':
                        ...
                        $def_attr = array( // defaults
                            'text' => Engine_config::$default_text,
                            'search_text' => Engine_config::$default_search_text,
                            'zoom' => Engine_config::$default_zoom,
                            'width' => Engine_config::$default_width,
                            'height' => Engine_config::$default_height,
                            'search' => Engine_config::$default_coord,
                            'coords' => Engine_config::$default_search);
                        $this->field_attr[$fitem['table'] . '.' . $fitem['field']] = array_merge($def_attr, (array )$attr);
                        break;
                    case 'remote_image':
    ...
    protected function _run_task()
    {
        ...
        switch ($this->task)
        {
            ...
            case 'print':
                ...
                $this->_set_field_types('list', Engine_config::$print_all_fields);
                $this->theme = 'printout';
                return $this->_list();
                break;
            case 'depend':
            ...
            case 'file':
                ...
                break;
            case 'csv':
                $this->_set_field_types('list', Engine_config::$csv_all_fields);
                return $this->_csv();
                break;
            case 'list':
            default:
    ...
    protected function render_custom_datagrid()
    {
        ...
        $db = Engine_db::get_instance($this->connection);
    ...
    protected function render_custom_csv()
    {
        ...
        fputcsv($output, $this->columns_names, Engine_config::$csv_delimiter, Engine_config::$csv_enclosure);
        ...
        while ($row = $db->result->fetch_assoc()) // low level result process, saves memory
        {
            ...
            fputcsv($output, $out, Engine_config::$csv_delimiter, Engine_config::$csv_enclosure);
        }
    }
    ...
    protected function render_output()
    {
        ...
        {
            $contents = '';
            if (!self::$css_loaded && !Engine_config::$manual_load)
            {
                $contents .= self::load_css();
            }
            ob_start();
            include (XCRUD_PATH . '/' . Engine_config::$themes_path . '/' . $this->theme . '/engine_container.php');
            $contents .= ob_get_contents();
            ob_end_clean();
            unset($this->data);
            if (!self::$js_loaded && !Engine_config::$manual_load)
            {
    ...
    protected function after_render()
    {
        ...
        if (is_callable(Engine_config::$after_render))
        {
            call_user_func(Engine_config::$after_render);
        }
    }
    ...
    public function _csv()
    {
        ...
        $output = fopen('php://output', 'w');
        fwrite($output, chr(0xEF) . chr(0xBB) . chr(0xBF)); // bom
        fputcsv($output, $headers, Engine_config::$csv_delimiter, Engine_config::$csv_enclosure);
    ...
    {
        if (isset($_POST['engine'][$field]))
        {
            if (get_magic_quotes_gpc())
            {
                if (is_array($_POST['engine'][$field]))
                {
                    array_walk_recursive($_POST['engine'][$field], array($this, 'stripslashes_callback'));
                }
                else
                {
                    $_POST['engine'][$field] == stripslashes($_POST['engine'][$field]);
                }
            }
            if (Engine_config::$auto_xss_filtering)
            {
            ...
            if (($field == 'postdata' or $field == 'unique') && $_POST['engine'][$field])
            {
                $data_keys = array_keys($_POST['engine'][$field]);
                foreach ($data_keys as $k => $key)
                {
                    $data_keys[$k] = $xss ? $xss->xss_clean($this->fieldname_decode($key)) : $this->fieldname_decode($key);
                    if ($xss)
                    {
                        $_POST['engine'][$field][$key] = $xss->xss_clean($_POST['engine'][$field][$key]);
                    }
                }
                return array_combine($data_keys, $_POST['engine'][$field]);
            }
            elseif ($filter)
            {
                switch ($filter)
                {
                    case 'key':
                        return str_replace('`', '', $xss ? $xss->xss_clean($_POST['engine'][$field]) : $_POST['engine'][$field]);
                        break;
                    case 'int':
                        return (int)$_POST['engine'][$field];
                        break;
                    case 'trim':
                        return trim($xss ? $xss->xss_clean($_POST['engine'][$field]) : $_POST['engine'][$field]);
                        break;
                    case 'base64':
                        return $xss ? $xss->xss_clean($this->fieldname_decode($_POST['engine'][$field])) : $this->fieldname_decode($_POST['engine'][$field]);
                        break;
                    default:
                        return $xss ? $xss->xss_clean($_POST['engine'][$field]) : $_POST['engine'][$field];
                        break;
                }
            }
            else
    ...
    {
        if (isset($_GET['engine'][$field]))
        {
            if (get_magic_quotes_gpc())
            {
                if (is_array($_GET['engine'][$field]))
                {
                    array_walk_recursive($_GET['engine'][$field], array($this, 'stripslashes_callback'));
                }
                else
                {
                    $_GET['engine'][$field] == stripslashes($_GET['engine'][$field]);
                }
            }
            if (Engine_config::$auto_xss_filtering)
            {
            ...
            if ($filter)
            {
                ...
                {
                    case 'key':
                        return str_replace('`', '', $xss ? $xss->xss_clean($_GET['engine'][$field]) : $_GET['engine'][$field]);
                        break;
                    case 'int':
                        return (int)$_GET['engine'][$field];
                        break;
                    case 'trim':
                        return trim($xss ? $xss->xss_clean($_GET['engine'][$field]) : $_GET['engine'][$field]);
                        break;
                    default:
                        return $xss ? $xss->xss_clean($_GET['engine'][$field]) : $_GET['engine'][$field];
                        break;
                }
            }
            else
    ...
    protected function _create($postdata = array())
    {
        ...
        if ($this->before_create)
        {
            ...
            {
                $postdata = new Engine_postdata($this->result_row, $this);
    ...
    protected function _save()
    {
        ...
        if (!$this->primary_val)
        {
            ...
            $pd = new Engine_postdata($postdata, $this);
            ...
            if ($this->alert_create)
            {
                foreach ($this->alert_create as $alert)
                {
                    ...
                    if (Engine_config::$email_enable_html)
                        $message = $alert['message'] . '<br /><br />' . "\r\n" . ($alert['link'] ? '<a href="' . $alert['link'] .
                            '" target="_blank">' . $alert['link'] . '</a>' : '');
                    else
                        $message = $alert['message'] . "\r\n\r\n" . ($alert['link'] ? $alert['link'] : '');
                    $this->send_email($send_to, $alert['subject'], $message, $alert['cc'], Engine_config::$email_enable_html);
                }
            }
            if ($this->mass_alert_create)
            {
                foreach ($this->mass_alert_create as $alert)
                {
                    ...
                    {
                        $this->send_email($row[$alert['email_column']], $alert['subject'], $message, array(), Engine_config::$email_enable_html);
                    }
                }
            }
    ...
    protected function _receive_post($task = false, $primary = false)
    {
        ...
        else
        {
            ...
            $this->limit = $this->_post('limit', ($this->limit ? $this->limit : Engine_config::$limit));
        ...
        if (is_array($this->table_info) && count($this->table_info))
        {
            ...
            foreach ($this->table_info as $table => $types)
            {
                foreach ($types as $row)
                {
                    ...
                    if (!$this->field_null[$field_index] && Engine_config::$not_null_is_required && !isset($this->validation_required[$field_index]))
                    {
    ...
    protected function _define_field_type($row, $field_index)
    {
        ...
        switch ($type)
        {
            ...
            case 'boolean':
                ...
                if ($max_l == 1 && Engine_config::$make_checkbox)
                {
            ...
            case 'longtext':
                ...
                if (!isset($this->no_editor[$field_index]) && Engine_config::$auto_editor_insertion)
            ...
            case 'enum':
                ...
                $this->field_type[$field_index] = Engine_config::$enum_as_radio ? 'radio' : 'select';
            ...
            case 'set':
                ...
                $this->field_type[$field_index] = Engine_config::$set_as_checkboxes ? 'checkboxes' : 'multiselect';
                $this->field_attr[$field_index]['values'] = $max_l;
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = $row['Default'];
                break;
            case 'point':
                ...
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = Engine_config::$default_point ? Engine_config::$default_point : '0,0';
                break;
        }
    }
    ...
    protected function _render_list()
    {
        ...
        if ($this->column === false)
        {
            ...
            elseif (!Engine_config::$search_all)
            {
                if ($this->search_columns)
                {
        ...
        $mode = 'list';
        $view_file = XCRUD_PATH . '/' . Engine_config::$themes_path . '/' . $this->theme . '/' . $this->load_view['list'];
    ...
    protected function _render_details($mode)
    {
        ...
        */
        ...
        if ($this->inner_table_instance && ($mode == 'view' or $mode == 'edit')) // restoring nested objects
        {
            foreach ($this->inner_table_instance as $inst_name => $field)
            {
                if (isset($this->result_row[$field]))
                {
                    ...
                    if ($mode == 'view' && Engine_config::$nested_readonly_on_view)
                    {
                    ...
                    $this->nested_rendered[$inst_name] =
                        '<div class="engine-nested-container engine-container"><div class="engine-ajax" id="engine-ajax-' . base_convert(rand(), 10,
                        36) . '">' . $instance->render('list') . '</div></div>';
                }
            }
        }

        $view_file = XCRUD_PATH . '/' . Engine_config::$themes_path . '/' . $this->theme . '/' . $this->load_view[$mode];
        $this->check_file($view_file, 'render');
        ob_start();
        include ($view_file);
        ...
        //$this->data .= '<div class="engine-nested-container engine-container"><div class="engine-ajax" id="engine-ajax-' .
        //    base_convert(rand(), 10, 36) . '">' . $instance->render('list') . '</div></div>';
        $this->nested_rendered[$inst_name] = '<div class="engine-nested-container engine-container"><div class="engine-ajax" id="engine-ajax-' .
        base_convert(rand(), 10, 36) . '">' . $instance->render('list') . '</div></div>';
        }
        }
        }*/
    ...
    protected function _pagination($total, $start, $limit, $numpos = 10, $numlr = 2)
    {
        if ($total > $limit)
        {
            ...
            '"><a href="javascript:;" class="engine-action" data-start="' . $limit1 . '">' . $i . '</a></li>';
            }
            }*/
    ...
    protected function _cut($string, $field, $wordsafe = true, $dots = true)
    {
        ...
        $string = html_entity_decode($string, ENT_QUOTES, Engine_config::$mbencoding);
        ...
        $slen = mb_strlen($strip_string, Engine_config::$mbencoding);
        if ($slen <= $len || (Engine_config::$print_full_texts && $this->theme == 'printout'))
        {
            return $this->output_string($string, $this->strip_tags, $safe);
        }
        if ($wordsafe)
        {
            $end = $len;
            while ((mb_substr($strip_string, --$len, 1, Engine_config::$mbencoding) != ' ') && ($len > 0))
            {
            }
            if ($len == 0)
            {
            ...
            return $this->output_string(mb_substr($strip_string, 0, $len, Engine_config::$mbencoding), false, $safe) . ($dots ?
                '&#133;' : '');
        }
        return $this->output_string(mb_substr($strip_string, 0, $len, Engine_config::$mbencoding), false, $safe) . ($dots ?
            '&#133;' : '');
    ...
    {
        return mb_convert_case(str_replace('_', ' ', $text), MB_CASE_TITLE, Engine_config::$mbencoding);
    }
    ...
    public function _export_vars()
    {
        ...
        {
            if (isset($_SESSION['lists']['engine_session']) && $_SESSION['lists']['engine_session'])
            {
                foreach ($_SESSION['lists']['engine_session'] as $s_key => $s_val)
                { // workaround on some servers session duplication
                    $old_time = isset($s_val['time']) ? (int)$s_val['time'] : 0;
                    if ($time > $old_time + Engine_config::$autoclean_timeout) // autocleaner

                        unset($_SESSION['lists']['engine_session'][$s_key]);
                }
            }
        }
        ...
        {
            $_SESSION['lists']['engine_session'][$inst_name][$item] = $this->{$item};
        }
        $_SESSION['lists']['engine_session'][$inst_name]['before'] = $this->find_prev_task();

        if (Engine_config::$alt_session)
        {
            $data = $this->encrypt($_SESSION['lists']['engine_session']);

            if (class_exists('Memcache'))
            {
                $mc = new Memcache();
                $mc->connect(Engine_config::$mc_host, Engine_config::$mc_port);
                $res = $mc->set(self::$sess_id, $data, false, Engine_config::$alt_lifetime * 60);
            }
            elseif (class_exists('Memcached'))
            {
                $mc = new Memcached();
                $mc->connect(Engine_config::$mc_host, Engine_config::$mc_port);
                $res = $mc->set(self::$sess_id, $data, Engine_config::$alt_lifetime * 60);
            }
            else
            {
                self::error('Can\'t use alternative session. Memcache(d) is not available');
            }
            unset($_SESSION['lists']['engine_session']);
            if (!$res)
            {
    ...
    public function import_vars($key = false)
    {
        if (Engine_config::$alt_session)
        {
            ...
            $_SESSION['lists']['engine_session'] = $this->decrypt($data[0], $data[1]);
            unset($data);
            if (!$_SESSION['lists']['engine_session'])
            {
        ...
        foreach ($this->params2save() as $item)
        {
            $this->{$item} = $_SESSION['lists']['engine_session'][$inst_name][$item];
        }
    ...
    {
        return '<span class="engine-none">' . $value . '</span>';
    }
    ...
    {
        if (Engine_config::$clickable_list_links)
        {
    ...
    protected function create_radio($name, $value = '', $tag = array())
    {
        ...
        $label_tag = array('tag' => 'label', 'class' => 'engine-radio-label');
    ...
    protected function create_checkboxes($name, $value = '', $tag = array())
    {
        ...
        $label_tag = array('tag' => 'label', 'class' => 'engine-checkboxes-label');
    ...
    protected function create_hidden($name, $value = '', $tag = array())
    {
        return $this->single_tag($tag + array(
            ...
            'type' => 'hidden',
            'value' => $value,
            'name' => $name), 'engine-input', $this->field_attr[$name], true);
    }
    ...
    protected function create_file($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        $out .= $this->open_tag('div', 'engine-upload-container'); // file and delete button
        $out .= $this->open_tag('div', $this->theme_config('grid_button_group'));
        if ($value)
        {
            //$out .= $this->open_tag('span', 'engine-file-name');
            ...
            $attr = array(
                ...
                'class' => 'engine-file-name engine-' . $ext,
                'target' => '_blank');
        ...
        {
            $out .= $this->open_tag('span', 'engine-nofile ' . $this->theme_config('no_file'));
        ...
        if (!isset($tag['readonly']) && !isset($tag['disabled']))
        {
            ...
            $attr = array(
                ...
                'value' => $value,
                'type' => 'hidden');
            $out .= $this->single_tag('input', 'engine-input', $attr, true);
            // upload button
    ...
    {
        $out = '';
        $out .= $this->open_tag('span', $this->theme_config('upload_button'), array('class' => 'engine-add-file'));
    ...
    protected function create_view_file($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        {
            $out .= $this->open_tag('span', 'engine-nofile');
            $out .= $this->lang('no_file') . $this->close_tag('span');
        }
        return $out;
    }
    protected function create_image($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        $out = ''; // upload container
        $out .= $this->open_tag('div', 'engine-upload-container'); // image and delete button
        if ($value)
        {
            $binary = isset($this->upload_config[$name]['blob']) ? true : false;
            if ($binary && !$is_upload)
        ...
        {
            $out .= $this->open_tag('div', $this->theme_config('grid_button_group'));
            $out .= $this->open_tag('span', 'engine-noimage ' . $this->theme_config('no_file'));
    ...
    protected function create_view_image($name, $value = '', $tag = array())
    {
        ...
        {
            $out .= $this->open_tag('span', 'engine-noimage');
            $out .= $this->lang('no_image') . $this->close_tag('span');
        }
    ...
    protected function create_point($name, $value = '', $tag = array())
    {
        ...
        if (!$value)
        {
            $value = Engine_config::$default_point ? Engine_config::$default_point : '0,0';
        }
    ...
    protected function create_view_point($name, $value = '', $tag = array())
    {
        ...
        if (!$value)
        {
            $value = Engine_config::$default_point;
        }
        if ($value)
        {
            $tag = array(
    ...
    {
        exit('<div class="engine-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
    ...
    protected function _upload_file()
    {
        ...
        if (isset($_FILES) && isset($_FILES['engine-attach']) && !$_FILES['engine-attach']['error'])
        {
            $file = $_FILES['engine-attach'];
    ...
    {
        $out = ''; // upload container
        $out .= $this->open_tag('div', 'engine-upload-container');
        ...
        $attr = array(
            ...
            'alt' => '');
        $out .= $this->single_tag('img', 'engine-crop', $attr);
    ...
    protected function _compare($val1, $operator, $val2)
    {
        switch ($operator)
        {
            ...
            case '!=':
                return ($val1 != $val2) ? true : false;
            case '^=':
                return (mb_strpos($val1, $val2, 0, Engine_config::$mbencoding) === 0) ? true : false;
            case '$=':
                return (mb_strpos($val1, $val2, 0, Engine_config::$mbencoding) == (mb_strlen($val1, Engine_config::$mbencoding) -
                    mb_strlen($val2, Engine_config::$mbencoding))) ? true : false;
            case '~=':
                return (mb_strpos($val1, $val2, 0, Engine_config::$mbencoding) !== false) ? true : false;
            default:
                return false;
        }
    }
    protected function create_modal($field, $content, $image = false)
    {
        ...
        $out .= $this->open_tag('a', 'engine_modal', $attr);
        if (Engine_config::$images_in_grid && $image)
        {
            $out .= $content;
        }
        else
        ...
        if (isset($this->field_type[$field]))
        {
            switch ($this->field_type[$field])
            {
                ...
                case 'image':
                    if ($value)
                    {
                        if (Engine_config::$images_in_grid)
                        {
                            $settings = $this->upload_config[$field];
                            ...
                            $out .= $this->single_tag('img', '', array(
                                ...
                                'style' => 'max-height: ' . Engine_config::$images_in_grid_height . 'px;'));
                        }
                        else
                        {
                            $out .= $this->open_tag('a', '', array('target' => '_blank', 'href' => isset($this->upload_config[$field]['url']) ? $this->
        ...
        if ($this->buttons)
        {
            foreach ($this->buttons as $button)
            {
                if ($this->is_button($button['name'], $row))
                {
                    ...
                    if (Engine_config::$button_labels)
                    {
    ...
    protected function file_link($field, $primary_val, $thumb = false, $crop = false, $filename = false)
    {
        ...
        if ($thumb !== false)
        {
            $params['engine']['thumb'] = $thumb;
        }
        if ($crop)
        {
            $params['engine']['crop'] = $crop;
        }
        if (Engine_config::$dynamic_session)
        {
            $params['engine']['sess_name'] = session_name();
        }
        //return Engine_config::$scripts_url . '/' . Engine_config::$ajax_uri . '?' . http_build_query($params);
		return Engine_config::$ajax_uri . '?' . http_build_query($params);
    }
    ...
    {
        return htmlspecialchars((string )$text, ENT_QUOTES, Engine_config::$mbencoding);
    }
    ...
    protected function send_email($to, $subject = '(No subject)', $message = '', $cc = array(), $html = true)
    {
        $header = 'MIME-Version: 1.0' . "\r\n" . 'Content-type: text/' . ($html ? 'html' : 'plain') . '; charset=UTF-8' . "\r\n" .
            'From: ' . Engine_config::$email_from_name . ' <' . Engine_config::$email_from . ">\r\n";
        if ($cc)
    ...
    protected function _cell_attrib($field, $value, $order, &$row, $is_sum = false, $row_color = false, $row_class = false)
    {
        ...
        if ($field == $order && $this->is_sortable)
            $column_class[] = 'engine-current';
        if ($is_sum)
            $column_class[] = 'engine-sum';
        if ($row_color)
        {
    ...
    {
        if (is_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/' . $this->language . '.ini'))
            self::$lang_arr = parse_ini_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/' . $this->language . '.ini');
        elseif (is_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/en.ini'))
            self::$lang_arr = parse_ini_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/en.ini');
        if ($this->set_lang)
        {
    ...
    {
        if (is_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/' . Engine_config::$language . '.ini'))
            self::$lang_arr = parse_ini_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/' . Engine_config::$language . '.ini');
        elseif (is_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/en.ini'))
            self::$lang_arr = parse_ini_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/en.ini');
    }
    protected function _get_theme_config()
    { // loads theme configuration from ini file
        if (is_file(XCRUD_PATH . '/' . Engine_config::$themes_path . '/' . $this->theme . '/engine.ini'))
            $this->theme_config = parse_ini_file(XCRUD_PATH . '/' . Engine_config::$themes_path . '/' . $this->theme . '/engine.ini');
        else
            self::error('engine.ini does not exist in your theme folder');
    }
    protected function lang($text = '')
    {
        $langtext = mb_convert_case($text, MB_CASE_LOWER, Engine_config::$mbencoding);
        return htmlspecialchars((isset(self::$lang_arr[$langtext]) ? self::$lang_arr[$langtext] : $text), ENT_QUOTES,
            Engine_config::$mbencoding);
    }
    protected function theme_config($text = '')
    {
        $text = mb_convert_case($text, MB_CASE_LOWER, Engine_config::$mbencoding);
        return htmlspecialchars((isset($this->theme_config[$text]) ? $this->theme_config[$text] : ''), ENT_QUOTES, Engine_config::
            $mbencoding);
    }
    ...
    public static function load_css()
    {
        ...
        if (self::$css_loaded)
        {
            self::error('Engine\'s styles already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }

        self::$css_loaded = true;
        if (Engine_config::$load_bootstrap)
        {
            $out .= '<link href="' . Engine_config::$scripts_url . '/' . Engine_config::$plugins_uri .
                '/bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" />';
        }
        if (Engine_config::$load_jquery_ui)
            //$out .= '<link href="' . Engine_config::$scripts_url . '/' . Engine_config::$plugins_uri .'/jquery-ui/jquery-ui.min.css" rel="stylesheet" type="text/css" />';
        $out .= '<link href="https://ajax.googleapis.com/ajax/libs/jqueryui/1.11.4/themes/smoothness/jquery-ui.css" rel="stylesheet" type="text/css" />';
        
		
		
		if (Engine_config::$load_jcrop)
            $out .= '<link href="' . Engine_config::$scripts_url . '/' . Engine_config::$plugins_uri .
                '/jcrop/jquery.Jcrop.min.css" rel="stylesheet" type="text/css" />';
        $out .= '<link href="' . Engine_config::$scripts_url . '/' . Engine_config::$plugins_uri .
            '/timepicker/jquery-ui-timepicker-addon.css" rel="stylesheet" type="text/css" /><link href="' . Engine_config::$scripts_url .
            '/' . Engine_config::$themes_uri . '/' . Engine_config::$theme . '/engine.css" rel="stylesheet" type="text/css" />';
        return $out;
    }
    public static function load_js()
    {
        ...
        if (self::$js_loaded)
        {
            self::error('Engine\'s scripts already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }
        self::$js_loaded = true;
        if (Engine_config::$load_jquery)
          $out .= '<script src="' . includes_url('js/jquery/jquery.js?ver=1.11.3') . '"></script>';
        $out .= '<script src="' . includes_url('js/jquery/jquery-migrate.min.js?ver=1.2.1') . '"></script>';
        
		
		if (Engine_config::$jquery_no_conflict)
        {
            $out .= '
        ...
        if (Engine_config::$load_jquery_ui)
           // $out .= '<script src="' . Engine_config::$scripts_url . '/' . Engine_config::$plugins_uri .'/jquery-ui/jquery-ui.min.js"></script>';
				
				$out .= '<script src="https://ajax.googleapis.com/ajax/libs/jqueryui/1.11.4/jquery-ui.min.js"></script>';
		
				
        if (Engine_config::$load_jcrop)
        {
            $out .= '<script src="' . Engine_config::$scripts_url . '/' . Engine_config::$plugins_uri .
        ...
        if (Engine_config::$editor_url)
            $out .= '<script src="' . Engine_config::$editor_url . '"></script>';
        if (Engine_config::$load_googlemap)
            $out .= '<script src="//maps.google.com/maps/api/js?sensor=false&language=' . $language . '"></script>';
        $out .= '<script src="' . Engine_config::$scripts_url . '/' . Engine_config::$plugins_uri . '/engine.js"></script>';

        $config = array(
            'url' => Engine_config::$ajax_uri,
            'editor_url' => Engine_config::$editor_url,
            'editor_init_url' => Engine_config::$editor_init_url,
            'force_editor' => Engine_config::$force_editor,
            'date_first_day' => Engine_config::$date_first_day,
            'date_format' => Engine_config::$date_format,
            'time_format' => Engine_config::$time_format,
            'lang' => self::$lang_arr,
            'rtl' => Engine_config::$is_rtl ? 1 : 0);
        $out .= '
            <script type="text/javascript">
            <!--
            
           	var engine_config = ' . json_encode($config) . ';
                            
            -->
            </script>';
        if ($language != 'en')
        {
            if (is_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/datepicker/jquery.ui.datepicker-' . $language . '.js'))
                $out .= '<script src="' . Engine_config::$scripts_url . '/' . Engine_config::$lang_uri .
                    '/datepicker/jquery.ui.datepicker-' . $language . '.js"></script>';
            if (is_file(XCRUD_PATH . '/' . Engine_config::$lang_path . '/timepicker/jquery-ui-timepicker-' . $language . '.js'))
                $out .= '<script src="' . Engine_config::$scripts_url . '/' . Engine_config::$lang_uri .
    ...
    protected function get_limit_list($limit = 20, $buttons = false)
    {
        ...
        if ($buttons)
        {
            $out .= $this->open_tag(array(
                'tag' => 'div',
                'class' => 'btn-group engine-limit-buttons',
            ...
            foreach ($this->limit_list as $limts)
            {
                ...
                $out .= $this->open_tag(array(
                    ...
                    'data-limit' => $limts), 'engine-action') . $this->lang($limts) . $this->close_tag(array('tag' => 'button'));
            }
        ...
        else
        {
            $out .= $this->open_tag(array(
                ...
                'name' => 'limit'), 'engine-actionlist engine-data');
            foreach ($this->limit_list as $limts)
            {
    ...
    {
        if (!Engine_config::$performance_mode)
        {
    ...
    {
        if (!is_array($value) && !Engine_config::$performance_mode && $value)
        {
    ...
    protected function send_http_request($url, $data, $method, $return_result = false)
    {
        ...
        {
            case 'get':
                $ch = curl_init($path . ((mb_strpos($path, '?', 0, Engine_config::$mbencoding) === false) ? '?' : '&') . $data);
                break;
            case 'post':
                $ch = curl_init($path);
        ...
        if (Engine_config::$use_browser_info)
        {
        ...
        'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        {
            $tabs_header = $this->open_tag($tabs_block, $this->theme_config('tabs_container'), array('class' => 'engine-tabs')) . $this->
            ...
            if ($this->nested_rendered && Engine_config::$nested_in_tab)
            {
    ...
    protected function get_table_tooltip()
    {
        ...
        if ($this->table_tooltip)
        {
            ...
            $out .= $this->open_tag(array(
                'tag' => 'a',
                'href' => 'javascript:;',
                'class' => 'engine-tooltip engine-button-link',
    ...
    protected function render_search()
    {
        ...
        if ($this->is_search)
        {
            ...
            $attr = array('class' => 'engine-search-toggle', 'href' => 'javascript:;');
            if ($this->search or Engine_config::$search_opened)
            {
                $attr['style'] = 'display:none;';
            }
            $out .= $this->open_tag('a', $this->theme_config('search_open'), $attr);
            $out .= $this->lang('search') . $this->close_tag('a');
            $attr = array('class' => 'engine-search');
            if (!$this->search && !Engine_config::$search_opened)
            {
                $attr['style'] = 'display:none;';
            }
            $out .= $this->open_tag('span', $this->theme_config('search_container'), $attr);
            if (Engine_config::$search_all)
            {
            ...
            $attr = array('class' => 'engine-data', 'name' => 'column');
            if ($this->search_default === false && !$this->search_columns)
            {
                $out .= $this->open_tag(array('tag' => 'input', 'type' => 'hidden'), 'engine-columns-select ' . $this->theme_config('search_fieldlist'),
                    $attr);
            }
            else
            {
                $out .= $this->open_tag('select', 'engine-columns-select ' . $this->theme_config('search_fieldlist'), $attr);
    ...
    {
        $attr_preset = array('class' => 'engine-searchdata', 'name' => 'phrase');
        if ($field == $this->column)
        {
            $class = 'engine-search-active';
        }
        else
        {
            $class = '';
        ...
        switch ($attr['data-type'])
        {
            ...
            case 'timestamp':
                if (!isset($fieldlist['date']) or $field == $this->column)
                {
                    ...
                    $attr_range = array(
                        'class' => 'engine-daterange engine-searchdata ' . $this->theme_config('search_range'),
                        'name' => 'range',
                        'data-fieldtype' => 'date');
                    if ($field != $this->column)
                    {
                    ...
                    $fieldlist['date'] .= $this->open_tag('option', '', array('value' => '')) . $this->lang('choose_range') . $this->
                        close_tag('option');
                    if (Engine_config::$available_date_ranges)
                    {
                        foreach (Engine_config::$available_date_ranges as $range)
                        {
                    ...
                    $fieldlist['date'] .= $this->single_tag('input', 'engine-datepicker-from ' . $class . ' ' . $this->theme_config('search_from'),
                        $attr);
                    $attr['name'] = 'phrase][to';
                    $attr['value'] = (isset($phrase['to']) && $field == $this->column) ? $phrase['to'] : '';
                    $fieldlist['date'] .= $this->single_tag('input', 'engine-datepicker-to ' . $class . ' ' . $this->theme_config('search_to'),
                        $attr);
                }
                break;
            case 'select':
    ...
    {
        $out = '';
        $out .= $this->open_tag($row, 'engine-th');
        if ($this->is_numbers)
        {
            $out .= $this->open_tag($item, 'engine-num') . '&#35;' . $this->close_tag($item);
        }
        ...
        {
            $out .= $this->open_tag($item, 'engine-actions') . '&nbsp;' . $this->close_tag($item);
        }
        foreach ($this->columns as $field => $fitem)
        {
            if (isset($this->field_type[$field]) && ($this->field_type[$field] == 'password' or $this->field_type[$field] ==
            ...
            if ($this->is_sortable)
            {
                ...
                {
                    $class .= ' engine-current engine-' . $this->order_direct;
    ...
    protected function render_grid_body($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        if ($this->result_list)
        {
            foreach ($this->result_list as $key => $row)
            {
                ...
                $out .= $this->open_tag($row_tag, 'engine-row engine-row-' . $i);
                if ($this->is_numbers)
                {
                    $out .= $this->open_tag($item, 'engine-num', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class)) . ($key +
                        $this->start + 1) . $this->close_tag($item);
                }
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'left')
                {
                    $out .= $this->open_tag($item, 'engine-actions', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class));
                ...
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'right')
                {
                    $out .= $this->open_tag($item, 'engine-actions' . (Engine_config::$fixed_action_buttons ? ' engine-fix' : ''), $this->
        ...
        else
        {
            ...
            $out .= $this->open_tag($row_tag, 'engine-row') . $this->open_tag($item, '', array('colspan' => $j)) . $this->lang('table_empty') .
    ...
    protected function render_grid_footer($row = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        {
            $out .= $this->open_tag($row, 'engine-tf');
            if ($this->is_numbers)
            {
                $out .= $this->open_tag($item, 'engine-num') . '&Sigma;' . $this->close_tag($item);
            }
    ...
    protected function render_benchmark($tag = array('tag' => 'span'))
    {
        if ($this->benchmark)
        {
            return $this->open_tag($tag, 'engine-benchmark') . $this->benchmark_end() . $this->close_tag($tag);
        }
        return '';
    }
    protected function render_control_fields()
    {
        ...
        $out .= $this->single_tag($tag, '', array('name' => 'limit', 'value' => ($this->limit ? $this->limit : Engine_config::$limit)));
    ...
    protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = '')
    {
        ...
        if (!isset($this->hide_button[$name]))
        {
            ...
            $out .= $this->open_tag($tag, 'engine-action');
            if ($icon && !$this->is_rtl)
            {
    ...
    protected function csv_button($class = '', $icon = '')
    {
        ...
        {
            return $this->render_button('export_csv', 'csv', '', $class . ' engine-in-new-window', $icon);
        }
    }
    protected function print_button($class = '', $icon = '')
    {
        ...
        {
            return $this->render_button('print', 'print', '', $class . ' engine-in-new-window', $icon);
        }
    }
    ...
    protected function get_image_folder($field)
    {
        ...
        {
            $path = $this->check_folder(Engine_config::$upload_folder_def, 'get_image_folder');
        }
    ...
    {
        $range = array();
        $time = time() /* + 3600 * Engine_config::$local_time_correction*/;
        $week_day = date('w', $time) /* + Engine_config::$date_first_day*/;
        switch ($name)
        {
            default:
            case 'today':
            ...
            case 'this_week_today':
                if ($week_day >= Engine_config::$date_first_day)
                {
                    $offset1 = $week_day - Engine_config::$date_first_day;
                }
                else
                {
                    $offset1 = 7 - (Engine_config::$date_first_day - $week_day);
                }
            ...
            case 'this_week_full':
                ...
                $offset2 = 6 - $week_day + Engine_config::$date_first_day;
                $range['from'] = gmmktime(0, 0, 0, date('n', $time), date('j', $time) - $offset1, date('Y', $time));
    ...
    {
        if ($time)
            return $utc ? gmdate($this->date_format['php_d'] . ' ' . $this->date_format['php_t'], $time) : date(Engine_config::$php_date_format .
    ...
    protected function render_table_name($mode = 'list', $tag = 'h2', $to_show = false)
    {
        ...
        if ($this->is_title)
        {
            ...
            if (Engine_config::$can_minimize)
            {
                if ($to_show)
                    $out .= '<span class="engine-toggle-show engine-toggle-down"><i class="' . $this->theme_config('slide_down_icon') .
                        '"></i></span>';
                else
                    $out .= '<span class="engine-toggle-show engine-toggle-up"><i class="' . $this->theme_config('slide_up_icon') .
                        '"></i></span>';
            }
            $out .= $this->close_tag($tag);
        }
    ...
    {
        if (!Engine_config::$alt_encription_key)
        {
        ...
        $key = substr(Engine_config::$alt_encription_key, 0, $ks);
    ...
    {
        $_SESSION['lists']['engine_session'] = $data;
    }
    public static function export_session()
    {
        return $_SESSION['lists']['engine_session'];
    }
...
class Engine_postdata
{
    private $engine = null;
    private $postdata = array();
    public function __construct($postdata, $engine)
    {
        $this->engine = $engine;
    ...
    {
        $fdata = $this->engine->_parse_field_names($name, 'Engine_postdata');
        foreach ($fdata as $key => $fitem)
        {
            $this->postdata[$key] = $value;
        }
        $this->engine->unlock_field($name);
        return $this;
    }
    ...
    public function get($name)
    {
        ...
        return /*new Engine_postdata_item*/ ($value);
    }
