...
//includes_url('js/jquery/jquery.js?ver=1.11.3')

//require_once ($engine_wp_load.'../../../../../../wp-load.php');
...
class Engine_config
{
    ...
    public static $load_jquery = true; // loads jQuery, turn it off if you already have jQuery on your page. jQuery version must be at least 1.7. If your jQuery loads in the bottom of page, you must activate $manual_load and use  Engine::load_css() & Engine::load_js() on your page.
    ...
    public static $manual_load = false; // Allows you to disable engines css and js output, but you can use Engine::load_css() & Engine::load_js() in your code manually.
    ...
    // engine folder url
    public static $scripts_url = ''; // URL to the xCRUD folder, not real path, without a trailing slash, can be relative, e.g. 'some_folder/engine' or absolute, e.g. 'http://www.your_site.com/some_folder/engine'. If empty - will be detected automatically
    ...
    // urls (relative to $scripts_url or engine's folder, if $scripts_url is not defined)
    ...
    public static $ajax_uri = WAW_AJAX_CMS_URL; // main ajax file or url
    //public $ajax_uri = site_url('engine_ajax.php'); // main ajax file or url
    // paths (relative to engine's folder)
    ...
    public static $autoclean_timeout = 3; // in seconds. Do not change, if not sure. Engine clears old instances in session when you reload browser tab or open new tab with engine. In this case Engine can't work in two tabs in the same time. You can increase timeout on your risk.
    
    
    // anti XSS
    public static $auto_xss_filtering = false; // enable all engine's POST and GET data filtering
